	...
	function realTime() {
		$.ajax({
			...
			success: function (result) {
				...
				orderStatus  = value.order_status_code;
				
				if(orderStatus == 200){
					orderDef = 'Order Placed Successfully';
				}
				else if(orderStatus == 500){
					orderDef = 'something went wrong';
				}
				else if(orderStatus == 400){
					orderDef = 'Market Closed or Missing or bad request parameters or values';
				}
				else if(orderStatus == 403){
					orderDef = 'Session expired or invalidate. Must relogin';
				}
				else if(orderStatus == 404){
					orderDef = 'Request resource was not found';
				}
				else if(orderStatus == 405){
					orderDef = 'Request method (GET, POST etc.) is not allowed on the requested endpoint';
				}
				else if(orderStatus == 410){
					orderDef = 'The requested resource is gone permanently';
				}
				else if(orderStatus == 429){
					orderDef = 'Too many requests to the API (rate limiting)';
				}
				else if(orderStatus == 502){
					orderDef = 'The backend OMS is down and the API is unable to communicate with it';
				}
				else if(orderStatus == 503){
					orderDef = 'Service unavailable; the API is down';
				}
				else if(orderStatus == 504){
					orderDef = 'Gateway timeout; the API is unreachable';
				}else{
					orderDef = 'Unknown Error';
				}
